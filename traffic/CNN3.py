...
def conv2d_6(x, W):
    return tf.nn.conv2d(x, W, strides=[1, 1, 42, 1], padding='SAME')
...
# 构建网络
# 获取数据+BN层1
x_image = tf.reshape(x, [-1, 10, 420, 1])
epsilon = 0.001
...
# 卷积层1+池化层1
W_conv1 = weight_variable([4, 42, 1, 32])
...
# 读入真实数据
input_count = 6494
x_s = np.loadtxt('Comnet-14_all_withoutIP+Port_CNN_train.txt')
y_s = np.loadtxt('Comnet-14_all_withoutIP+Port_CNN_train_label.txt')
train_images = np.array([[0] * 4200 for i in range(input_count)])
...
input_count1 = 696
x_t = np.loadtxt('Comnet-14_all_withoutIP+Port_CNN_test.txt')
y_t = np.loadtxt('Comnet-14_all_withoutIP+Port_CNN_test_label.txt')
test_images = np.array([[0] * 4200 for i in range(input_count1)])
